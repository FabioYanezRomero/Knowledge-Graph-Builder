...
- Ground EVERY salient entity as a node. Emit a (head, relation, tail) triple
  ...
  text does not state. If an entity has no stated relationship, emit it standalone
  with empty relation/tail: {"head": "<entity>", "relation": "", "tail": ""}.
  Never drop a mentioned entity.
